...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        '--task',
        choices=['foundation', 'flood', 'foundation_xdxd_sn5', 'foundation_xdxd_sn4'],
        required=True
    )
    parser.add_argument(
        '--exp_id',
...
def get_default_cfg_path(task: str) -> str:
    ...
    elif task == 'foundation_xdxd_sn4':
        return 'configs/defaults/foundation_xdxd_sn4.yaml'
    else:
...
def main() -> None:
    ...
    model = get_model(config, model_dir, pretrained_exp_id=args.pretrained, pretrained_xdxd_path=args.pretrained_xdxd_path)
