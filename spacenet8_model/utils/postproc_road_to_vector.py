...
def make_skeleton(img_loc, thresh, debug, fix_borders, replicate=5,
    ...
    img = ds.ReadAsArray().astype(float) / 255.0
    img = img[skelton_bands, :, :].max(axis=0)  # use higher one from 'road' and 'road_junction' pred scores
    img = (img >= road_thresh).astype(np.uint8) * 255
